...
# 1. Core servisleri başlat (backend bağımlı)
...
#    Entrypoint SKIP_DB_MIGRATIONS=1 ile migration atlar, CMD olarak migration komutu çalışır.
#    P3005 (mevcut DB, migration history yok) durumu da ele alınır.
...
#    SKIP_DB_MIGRATIONS=1 → backend entrypoint migration döngüsüne girmez
