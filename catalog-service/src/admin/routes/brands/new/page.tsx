...
const BrandCreatePage = () => {
  ...
  const handleCreateBrand = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        toast.success("Başarılı", { description: "Marka oluşturuldu" })
        navigate(`/brands/${data.brand.id}`)
      } else {
        const errorData = await response.json().catch(() => ({}))
        toast.error("Hata", { 
          description: errorData.message || "Marka oluşturulurken hata oluştu" 
        })
      }
    } catch (error) {
      console.error("Marka oluşturma hatası:", error)
      toast.error("Hata", { description: "Marka oluşturulurken hata oluştu" })
    } finally {
      setSaving(false)
    }
  }
      ...
      <Container className="divide-y p-0">
        <div className="flex items-center gap-4 px-6 py-4">
          ...
          <div className="flex-1">
            <Heading level="h1" className="text-ui-fg-base">Yeni Marka Oluştur</Heading>
            <Text size="small" className="text-ui-fg-subtle mt-1">
              Yeni bir marka ekleyin
            </Text>
          </div>
