/**
 * Kullanıcı profil banner görseli yükleme scripti
...
async function uploadUserBanner(credentials: UserCredentials, imagePath: string) {
  try {
    ...
    const fileName = `profile-banners/${user.id}/${uuidv4()}.${fileExtension}`;
    
    const fileUrl = await s3Service.uploadFile(fileName, imageBuffer, contentType);
    console.log(`✅ Banner görseli yüklendi: ${fileUrl}`);
    ...
    if (existingProfile) {
      ...
      console.log(`✅ Profil güncellendi: ${existingProfile.id}`);
    } else {
      // Profile yoksa oluştur
      await prisma.profile.create({
        data: {
          userId: user.id,
          displayName: user.profile?.displayName || (user.email ? user.email.split('@')[0] : 'User'),
          bannerUrl: fileUrl,
        },
      });
    ...
    console.log(`   Banner URL: ${fileUrl}`);
    ...
    if (updatedUser?.profile?.bannerUrl) {
      console.log(`   Banner URL: ${updatedUser.profile.bannerUrl}`);
    } else {
      console.log(`   Banner URL: Yok`);
    }
...
const imagePath = join(__dirname, '../tests/assets/userprofile/banner.png');

console.log('🚀 Kullanıcı profil banner görseli yükleme scripti başlatılıyor...\n');
uploadUserBanner(userCredentials, imagePath);
