...
const COMMUNITY_COACH_AVATAR_URL = getSeedMediaPath('user.avatar.truster3', true) || ''
...
async function main() {
  ...
  // ============================================================
  // ADIM 1: TÜM SEED GÖRSELLERİNİ MİNİO'YA YÜKLE
  // ============================================================
  // ÖNEMLİ: Bu adım seed verileri DB'ye yazılmadan ÖNCE yapılmalı
  // - Bucket yoksa oluşturulur
  // - Tüm görseller MinIO'ya yüklenir
  // - Hata olursa seed durdurulur (görseller kritik)
  // ============================================================
  
  if (process.env.SKIP_SEED_MEDIA_UPLOAD !== 'true') {
    progress.increment('Seed görselleri MinIO\'ya yükleniyor...')
    console.log('\n═══════════════════════════════════════════════════════════')
    console.log('📦 ADIM 1: SEED GÖRSELLERİNİ MİNİO\'YA YÜKLEME')
    console.log('═══════════════════════════════════════════════════════════\n')
    console.log('ℹ️  Bu adımda:')
    console.log('   1. MinIO bucket kontrol edilir (yoksa oluşturulur)')
    console.log('   2. Tüm seed görselleri MinIO\'ya yüklenir')
    console.log('   3. Hangi klasörlerin oluşturulduğu gösterilir')
    console.log('   4. Başarılı olursa seed verileri DB\'ye yazılır\n')
    
    try {
      console.log('📦 Seed görselleri yükleme başlatılıyor...\n')
      await ensureSeedMediaUploaded();
      console.log('\n✅ ADIM 1 TAMAMLANDI: Tüm seed görselleri başarıyla MinIO\'ya yüklendi!')
      console.log('✅ MinIO bucket ve klasörler hazır')
      console.log('✅ Şimdi seed verileri DB\'ye yazılacak...\n')
      console.log('═══════════════════════════════════════════════════════════\n')
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
      console.error('\n❌ ADIM 1 BAŞARISIZ: Seed görselleri yüklenirken kritik hata oluştu!')
      console.error('═══════════════════════════════════════════════════════════')
      console.error(`   Hata: ${errorMsg}`)
      if (error instanceof Error && error.stack) {
        console.error(`   Stack: ${error.stack}`)
      }
      console.error('\n   ⚠️  Seed işlemi durduruluyor!')
      console.error('   💡 Görseller MinIO\'ya yüklenmeden seed verileri DB\'ye yazılamaz.')
      console.error('   💡 MinIO bağlantısını ve bucket yapılandırmasını kontrol edin.')
      console.error('═══════════════════════════════════════════════════════════\n')
      throw error; // Seed'i durdur - görseller kritik
    }
  } else {
    console.log('⏭️  Seed görsel yükleme atlandı (SKIP_SEED_MEDIA_UPLOAD=true)')
    console.log('⚠️  UYARI: Görseller yüklenmedi, seed verileri eksik görsellerle yazılacak!\n')
    progress.increment('Seed görselleri yükleme atlandı...')
  }

  // ============================================================
  // ADIM 2: SEED VERİLERİNİ DATABASE'E YAZ
  // ============================================================
  // Bu adımda:
  // - Tüm seed verileri DB'ye yazılır
  // - Görsel path'leri (MinIO'dan) kullanılır
  // - getSeedMediaPath() ile path'ler alınır
  // ============================================================
  
  console.log('═══════════════════════════════════════════════════════════')
  console.log('💾 ADIM 2: SEED VERİLERİNİ DATABASE\'E YAZMA')
  console.log('═══════════════════════════════════════════════════════════\n')
  console.log('ℹ️  Bu adımda:')
  console.log('   1. Taxonomy verileri (kategoriler, brand categories, vb.)')
  console.log('   2. User ve profile verileri')
  console.log('   3. Product ve content verileri')
  console.log('   4. Tüm görsel path\'leri MinIO\'dan alınır ve DB\'ye yazılır\n')
  ...
  // ============================================================
  // SEED İŞLEMİ TAMAMLANDI - ÖZET
  // ============================================================
  console.log('\n═══════════════════════════════════════════════════════════')
  console.log('✅ SEED İŞLEMİ BAŞARIYLA TAMAMLANDI!')
  console.log('═══════════════════════════════════════════════════════════\n')
  console.log('📊 Özet:')
  console.log('   ✅ ADIM 1: Tüm seed görselleri MinIO\'ya yüklendi')
  console.log('   ✅ ADIM 2: Tüm seed verileri database\'e yazıldı')
  console.log('   ✅ MinIO bucket ve klasörler hazır')
  console.log('   ✅ Database verileri hazır')
  console.log('\n💡 Tüm görseller MinIO\'da mevcut ve path\'ler DB\'de kayıtlı.')
  console.log('💡 Seed verileri artık kullanıma hazır!\n')
  console.log('═══════════════════════════════════════════════════════════\n')
