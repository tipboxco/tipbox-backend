/**
 * Interaktif Mesajlaşma Test Script'i - Tuna & Georgia
 * 
 * Tuna ve Georgia kullanıcıları arasında tüm mesajlaşma senaryolarını test eder.
 * REST API ve Socket.IO event'lerini kapsar.
 * 
 * Kullanım:
 *   npm run test:tuna-georgia
 *   veya
 *   docker exec -it tipbox_backend npm run test:tuna-georgia
 */
...
/**
 * Thread ID'den Support Request ID bul
 */
async function getSupportRequestIdFromThread(api: AxiosInstance, threadId: string): Promise<string | null> {
  try {
    log.info(`Thread ID'den support request aranıyor: ${threadId}`);
    
    // Support request'leri listele (tüm status'ler dahil)
    ...
    if (response.data && response.data.items) {
      log.info(`Toplam ${response.data.items.length} support request bulundu`);
      
      // Thread ID'ye göre ara
      const request = response.data.items.find((req: any) => req.threadId === threadId);
      
      if (!request) {
        // Debug: Tüm thread ID'leri listele
        const threadIds = response.data.items
          .map((req: any) => ({ id: req.id, threadId: req.threadId, status: req.status }))
          .filter((req: any) => req.threadId);
        log.warning(`Thread ID'ye ait support request bulunamadı: ${threadId}`);
        if (threadIds.length > 0) {
          log.info(`Mevcut thread ID'ler: ${JSON.stringify(threadIds, null, 2)}`);
        }
        return null;
      }

      log.success(`Support Request ID bulundu: ${request.id} (Status: ${request.status}, ThreadId: ${request.threadId})`);
      return request.id;
    }

    log.error('Support request listesi alınamadı veya boş');
    return null;
  } catch (error: any) {
    log.error(`Support request sorgulama hatası: ${error.response?.data?.message || error.message}`);
...
async function closeSupportRequest(api: AxiosInstance): Promise<boolean> {
  try {
    ...
    if (!requestId) {
      const input = await question('Support Request ID veya Thread ID girin (Enter ile thread ID kullan): ');
      if (!input) {
        // Thread ID kullan
        const threadId = state.currentSupportThreadId;
        if (!threadId) {
          const threadIdInput = await question('Thread ID girin: ');
          if (!threadIdInput) {
            log.error('Request ID veya Thread ID gerekli!');
            return false;
          }
          requestId = await getSupportRequestIdFromThread(api, threadIdInput);
        } else {
          requestId = await getSupportRequestIdFromThread(api, threadId);
        }
      } else {
        // Önce thread ID olarak dene (daha yaygın kullanım)
        requestId = await getSupportRequestIdFromThread(api, input);
        
        // Thread ID olarak bulunamazsa, request ID olarak dene
        if (!requestId) {
          log.info(`Thread ID olarak bulunamadı, request ID olarak deneniyor: ${input}`);
          requestId = input; // Direkt request ID olarak kullan
        }
      }
      
      if (!requestId) {
        log.error('Request ID bulunamadı!');
        return false;
      }
    }
...
async function reportSupportRequest(api: AxiosInstance): Promise<boolean> {
  try {
    ...
    console.log('\nRapor Sebebi:');
    console.log('  1. SPAM');
    console.log('  2. HARASSMENT');
    console.log('  3. SCAM');
    
    const reasonChoice = await question('Sebep seçin (1-3): ');
    let reason = 'SPAM';
    switch (reasonChoice) {
...
async function main() {
  try {
    ...
    let currentUserId = userChoice === '1' ? TUNA_USER_ID : GEORGIA_USER_ID;
    let currentUserEmail = userChoice === '1' ? TUNA_EMAIL : GEORGIA_EMAIL;
    let currentUserName = userChoice === '1' ? 'Tuna' : 'Georgia';
    let otherUserId = userChoice === '1' ? GEORGIA_USER_ID : TUNA_USER_ID;
    let otherUserName = userChoice === '1' ? 'Georgia' : 'Tuna';
    ...
    if (currentUserName === 'Tuna') {
      ...
      state.georgiaToken = token;
      state.georgiaSocket = socket || undefined;
    }
    state.currentUser = currentUserName.toLowerCase() as 'tuna' | 'georgia';
    ...
    while (true) {
      ...
      if (choice === 'q' || choice === '0') {
        if (choice === '0') {
          ...
          currentUserId = userChoice === '1' ? TUNA_USER_ID : GEORGIA_USER_ID;
          currentUserEmail = userChoice === '1' ? TUNA_EMAIL : GEORGIA_EMAIL;
          currentUserName = userChoice === '1' ? 'Tuna' : 'Georgia';
          otherUserId = userChoice === '1' ? GEORGIA_USER_ID : TUNA_USER_ID;
          otherUserName = userChoice === '1' ? 'Georgia' : 'Tuna';
          ...
          if (currentUserName === 'Tuna') {
            ...
            state.georgiaToken = newToken;
            state.georgiaSocket = newSocket || undefined;
          }
          state.currentUser = currentUserName.toLowerCase() as 'tuna' | 'georgia';
      ...
      const currentSocket = currentUserName === 'Tuna' ? state.tunaSocket : state.georgiaSocket;
    ...
    if (state.georgiaSocket) {
      state.georgiaSocket.disconnect();
    }
    rl.close();
  } catch (error: any) {
